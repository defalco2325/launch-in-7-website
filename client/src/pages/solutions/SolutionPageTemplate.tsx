  ...
  return <div className="l7-solution">
    <section className="l7-solution-hero">
      ...
      <div className="l7-solution-hero-inner">
        ...
        <p className="l7-solution-index">SYSTEM / 0{Math.min(title.length % 6 + 1, 6)}</p>
        <h1>{title}</h1>
    ...
    <section className="l7-solution-section l7-solution-problems"><div className="l7-solution-label">01 / THE OPPORTUNITY</div><div className="l7-solution-heading"><h2>Remove the friction<br /><em>between intent and action.</em></h2><p>Good systems make the next useful move obvious — for your team and for the people you serve.</p></div><div className="l7-problem-grid">{problems.map((problem, i) => <article key={problem.title}><span>0{i + 1}</span><h3>{problem.title}</h3><p>{problem.description}</p></article>)}</div></section>
    <section className="l7-solution-section l7-solution-build"><div className="l7-solution-label">02 / THE BUILD</div><div className="l7-solution-heading"><h2>Useful infrastructure.<br /><em>Nothing ornamental.</em></h2><p>We build the pieces that make the outcome easier to create, measure, and improve.</p></div><div className="l7-build-list">{builds.map((build, i) => <article key={build.title}><span>{String(i + 1).padStart(2, "0")}</span><div><h3>{build.title}</h3><p>{build.description}</p></div><Check size={17} /></article>)}</div></section>
    <section className="l7-solution-method"><div className="l7-solution-section"><div className="l7-solution-label light">03 / THE METHOD</div><div className="l7-solution-heading"><h2>Map it. Make it.<br /><em>Make it better.</em></h2><p>Clear decisions, clean execution, a handover your team can actually use.</p></div><div className="l7-method-list">{howItWorks.map(step => <article key={step.step}><span>{step.step}</span><h3>{step.title}</h3><p>{step.description}</p></article>)}</div></div></section>
